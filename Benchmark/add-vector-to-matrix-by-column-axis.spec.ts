...
(async () => {
  ...
  const addVectorToMatrixByColumnAxisInRowMode = (
  ...
  const addVectorToMatrixByColumnAxisInColMode = (
    m: MatrixType | NumericMatrix,
    v: MatrixType | NumericMatrix,
  ) =>
    numeric.pointwise2(
  ...
  const condition = Matrix.FrobeniusNorm(
        Matrix.minus(
          ...
          addVectorToMatrixByColumnAxisInRowMode(r2, v1),
        ),
      ) <= 1e-8 && Matrix.FrobeniusNorm(
        Matrix.minus(
          Matrix.addVectorToMatrixByColumnAxis(r1, v2, undefined, "column"),
          addVectorToMatrixByColumnAxisInColMode(r2, v2),
        ),
      ) <= 1e-8;
  const euriklisTestForRowMode = (m: any) =>
    m.addVectorToMatrixByColumnAxis(r1, v1);
  const euriklisTestForColumnMode = (m: any) =>
    m.addVectorToMatrixByColumnAxis(r1, v2, undefined, "column");
