...
(async () => {
  ...
  const sumOfSquaresOfColumnElementsExceptDiagonalAsColumn = (m: MatrixType | NumericMatrix) =>
    numeric.mapreduce2(
  ...
  const condition = Matrix.FrobeniusNorm(
    Matrix.minus(
      ...
      Matrix.minus(
        Matrix.sumOfSquaresOfColumnElementsExceptDiagonal(r2, undefined, "column"),
        sumOfSquaresOfColumnElementsExceptDiagonalAsColumn(r2)
      )
    ) <= 1e-8;
  const euriklisTestAsRow = (m: any) => m.sumOfSquaresOfColumnElementsExceptDiagonal(r1);
  const euriklisTestAsColumn = (m: any) => m.sumOfSquaresOfColumnElementsExceptDiagonal(r1, undefined, "column");
  const numericTestAsRow = (_: any) => sumOfSquaresOfColumnElementsExceptDiagonalAsRow(r2);
  const numericTestAsColumn = (_: any) => sumOfSquaresOfColumnElementsExceptDiagonalAsColumn(r2);
