...
const divideVector2MatrixByRowAxis = r1.map((row: number[], i: Integer) => {
  return row.map((el: number) => el / v[0][i]);
});

const callDivideVectorToMatrix = (
  m: MatrixType | NumericMatrix,
  v: MatrixType | NumericMatrix,
) => Matrix.pointwiseDivideMatrixWithVectorByRowAxis(m, v);
new validator(
  Matrix.FrobeniusNorm(
    Matrix.minus(
      Matrix.pointwiseDivideMatrixWithVectorByRowAxis(r1, v),
      divideVector2MatrixByRowAxis,
    ),
  ) <= 1e-8,
).describe("The poitwiseDivideMatrixWithVectorByRowAxis method has to:")
...
new validator(callDivideVectorToMatrix)
