...
new validator(new StaticStack())
  .describe(
    "The StaticStack type is an instance which represents the Stack data structure by nodes and has to:",
  )
  .test({ title: true, error: "red", success: "green" });

let DStack = new StaticStack();
...
new validator(DStack.filter((el) => el > 15).list)
  ...
  .describe(
    "3. provide a correct method for filtering through the elements of the Stack which returns a StaticStack.",
  )
  .test();
...
DStack = new StaticStack().pushMany(
...
new validator(DStack.popMany(5))
  ...
  .on(false, (v) => console.log(v.value, DStack))
