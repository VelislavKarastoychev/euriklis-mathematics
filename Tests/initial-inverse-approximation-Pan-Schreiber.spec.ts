...
type inverseStepNameType =
  ...
  | "Li and Li"
  | "Soleymani";
const initializationMatrixName = "Pan and Schreiber";
const m = Matrix.uniqueRandom(100, 100, -10.00, 10.0);
// const invm = Matrix.inverse(m);
const PS1 = Matrix.initialInverseApproximationPanSchreiber(m);
...
const runInverseStep = (inverseStepName: inverseStepNameType) => {
  ...
  try {
    ...
    new validator(false).isSame(true).describe(`${inverseStepName} step fail.`)
      .test();
  }
};
